# fish completion for veribi                               -*- shell-script -*-
...
function __veribi_perform_completion
    __veribi_debug "Starting __veribi_perform_completion"
    ...
    __veribi_debug "args: $args"
    __veribi_debug "last arg: $lastArg"

    # Disable ActiveHelp which is not supported for fish shell
    set -l requestComp "VERIBI_ACTIVE_HELP=0 $args[1] __complete $args[2..-1] $lastArg"
    ...
    __veribi_debug "Comps: $comps"
    __veribi_debug "DirectiveLine: $directiveLine"
    __veribi_debug "flagPrefix: $flagPrefix"
...
function __veribi_prepare_completions
    __veribi_debug ""
    ...
    if test $compErr -eq 1
        __veribi_debug "Received error directive: aborting."
        # Might as well do file completion, in case it helps
        return 1
    end
    ...
    if test $filefilter -eq 1; or test $dirfilter -eq 1
        __veribi_debug "File extension filtering or directory filtering not supported"
    ...
    __veribi_debug "nospace: $nospace, nofiles: $nofiles"
    ...
    if test $nospace -ne 0; or test $nofiles -eq 0
        ...
        __veribi_debug "prefix: $prefix"

        set -l completions (string match -r -- "^$prefix.*" $__veribi_comp_results)
        set --global __veribi_comp_results $completions
        __veribi_debug "Filtered completions are: $__veribi_comp_results"

        # Important not to quote the variable for count to work
        set -l numComps (count $__veribi_comp_results)
        __veribi_debug "numComps: $numComps"
        ...
        if test $numComps -eq 1; and test $nospace -ne 0
            ...
            set -l split (string split --max 1 \t $__veribi_comp_results[1])
            ...
            if not string match -r -q "[@=/:.,]" -- "$lastChar"
                ...
                __veribi_debug "Adding second completion to perform nospace directive"
                set --global __veribi_comp_results $split[1] $split[1].
                __veribi_debug "Completions are now: $__veribi_comp_results"
            end
        end
        ...
        if test $numComps -eq 0; and test $nofiles -eq 0
            ...
            __veribi_debug "Requesting file completion"
            return 1
        end
    end
...
if type -q "veribi"
    ...
    complete --do-complete "veribi " > /dev/null 2>&1
end
